...
export function getOrderProgress(status: OrderStatus): number {
  ...
  const statusOrder: (typeof ORDER_STATUSES)[keyof Pick<
...
export function isOrderCancellable(status: OrderStatus): boolean {
  return [ORDER_STATUSES.PENDING, ORDER_STATUSES.CONFIRMED].includes(
    status as any
  );
}
...
export function isVerificationCodeExpired(
  ...
  const expiryTime = new Date(
    orderCreatedAt.getTime() + expiryHours * 60 * 60 * 1000
  );
