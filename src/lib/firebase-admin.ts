...
if (!admin.apps.length) {
  try {
    ...
    const serviceAccount = {
      ...
      project_id: process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || 'aastatechnology',
      private_key_id: process.env.FIREBASE_PRIVATE_KEY_ID,
      private_key: process.env.FIREBASE_PRIVATE_KEY?.replace(/\\n/g, '\n'),
      client_email: process.env.FIREBASE_CLIENT_EMAIL,
      client_id: process.env.NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID, // Use client's messagingSenderId
    ...
    admin.initializeApp({
      ...
      projectId: process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || 'aastatechnology',
    });
