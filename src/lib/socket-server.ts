...
export class SocketManager {
  ...
  private broadcastLocationUpdate(
    ...
    this.io
      .to(`delivery_partner_${deliveryPartnerId}`)
  ...
  private notifyOrderStatusUpdate(order: any) {
    ...
    // Notify customer
    this.io
      .to(`user_${order.customer.userId}`)
      .emit('order_status_update', orderUpdate);

    // Notify delivery partner if assigned
    if (order.deliveryPartner) {
      this.io
        .to(`user_${order.deliveryPartner.userId}`)
        .emit('order_status_update', orderUpdate);
    }
  ...
  public sendRestaurantNotification(
    restaurantId: string,
    type: string,
    data: any
  ) {
