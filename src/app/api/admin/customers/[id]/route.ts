...
export async function GET(
  ...
  try {
    ...
    const categoryFrequency = orders.reduce(
      (acc, order) => {
    ...
    const hourFrequency = orderHours.reduce(
      (acc, hour) => {
    ...
    const peakOrderingHour = Object.entries(hourFrequency).sort(
      ([, a], [, b]) => b - a
    )[0];
    ...
    const addressUsage = addresses.map((address) => ({
      ...
      orderCount: orders.filter(
        (order) => order.deliveryAddressId === address.id
      ).length,
    }));
    ...
    const orderFrequency =
      totalOrders > 0 && daysSinceFirstOrder > 0
