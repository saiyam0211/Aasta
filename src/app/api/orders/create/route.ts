...
export async function POST(request: NextRequest) {
  try {
    ...
    if (subtotal < restaurant.minimumOrderAmount) {
      return NextResponse.json(
        { success: false, error: `Minimum order amount is ₹${restaurant.minimumOrderAmount}` },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (isValidCoordinate(restaurantLat, restaurantLng) && 
        ...
        const deliveryCalculation = await googleMapsService.calculateDeliveryMetrics(
          restaurantLat,
          restaurantLng,
          customerLat,
          customerLng,
        ...
        calculatedDeliveryTime = deliveryCalculation.estimatedDeliveryTime || null;
        ...
        console.log(`Estimated delivery time: ${calculatedDeliveryTime?.toISOString()}`);
      } else {
        console.warn(`Invalid coordinates for distance calculation. Restaurant: ${restaurantLat}, ${restaurantLng}, Customer: ${customerLat}, ${customerLng}`);
      }
    } catch (mapsError) {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        estimatedDeliveryTime: calculatedDeliveryTime || new Date(Date.now() + 45 * 60 * 1000), // Use calculated time or fallback to 45 minutes
        ...
        orderItems: {
          create: cart.items.map((item: any) => {
            ...
            return {
              ...
              restaurantEarningsPerItem: originalPrice * (restaurant.restaurantPricePercentage || 0.4),
              restaurantTotalEarnings: originalPrice * (restaurant.restaurantPricePercentage || 0.4) * item.quantity,
              aastaEarningsPerItem: originalPrice * (restaurant.aastaPricePercentage || 0.1),
              aastaTotalEarnings: originalPrice * (restaurant.aastaPricePercentage || 0.1) * item.quantity,
            };
          })
        },
      },
            ...
            for (const partner of availablePartners) {
              if (partner.telegramPhone) {
                try {
                  ...
                  notificationsSent++;
                  console.log(`Order notification sent to partner ${partner.user?.name}`);
                } catch (notificationError) {
                  console.error(`Failed to send Telegram notification to partner ${partner.user?.name}:`, notificationError);
                }
              }
            }
            
            console.log(`Order ${order.orderNumber} notifications sent to ${notificationsSent} available delivery partners`);
          }
        } else {
          console.log(`No available delivery partners found for restaurant ${restaurant.name}. Order ${order.orderNumber} will remain unassigned.`);
        }
      } else {
        console.log(`No delivery partners assigned to restaurant ${restaurant.name}. Order ${order.orderNumber} will remain unassigned.`);
      }
    } catch (assignmentError) {
      console.error('Error sending notifications to delivery partners:', assignmentError);
