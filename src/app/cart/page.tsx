...
export default function CartPage() {
  ...
  const computedTotal = useMemo(() => {
    return aastaSubtotal + platformFee + handlingCharge + packagingFeeDisplay + deliveryFee;
  }, [aastaSubtotal, platformFee, handlingCharge, packagingFeeDisplay, deliveryFee]);
    ...
    <CustomerLayout hideHeader hideFooter>
      <div className="min-h-screen bg-white">
        ...
        <div className="sticky top-0 z-10 bg-white px-4 pt-4">
          ...
          <div className="px-4 pb-32">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                     <p className="text-sm font-medium text-gray-900">
                       {mode === 'delivery' ? 'Delivery' : 'Pickup'}{etaText ? ` in ${etaText}` : ''}
                     </p>
                     <p className="text-xs text-gray-500">{etaText ? 'Live estimate' : 'Time shown at checkout'}</p>
                  </div>
                </div>
              </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900">Deliver to Home</p>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="text-sm font-medium text-gray-900">Bill Details</p>
                    <p className="text-xs text-gray-500">View detailed bill breakdown</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-gray-900">₹{Math.round(computedTotal)}</p>
                  {savings > 0 && (
                    <p className="text-xs text-green-600">Saved ₹{Math.round(savings)}</p>
                  )}
                </div>
              </button>
            </div>
            ...
            {mode === "delivery" && isEditingAddress && (
              <div className="mb-6 rounded-xl border border-orange-200 bg-orange-50 p-4">
                <h3 className="mb-3 text-sm font-medium text-gray-900">Update delivery address</h3>
                <LocationInput
            ...
            {/* Cart Items */}
            <div className="mb-6">
            <h2 className="-mb-1 text-lg font-semibold text-gray-900 flex items-center">
                <span className={`${brandFont.className} text-[60px]`} style={{letterSpacing: '-0.08em'}}>Items</span> 
                <span className="text-3xl ml-2 text-orange-500"> ({cart.items.length})</span>
              </h2>
              <div className="space-y-4">
                {cart.items.map((item, index) => (
                  <div key={item.menuItemId} className="rounded-xl border border-gray-200 bg-white p-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-3 flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              className="flex h-8 w-8 items-center justify-center text-red-500 hover:bg-red-50 rounded-xl"
                      onClick={() => {
          ...
          {/* Sticky Checkout Bar */}
          <div className="fixed bottom-20  left-1/2 w-full max-w-md -translate-x-1/2 border-t border-gray-200 bg-white px-4 py-4 shadow-t-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-wide text-gray-500">Total</p>
                <p className="text-xl font-bold text-gray-900">₹{Math.round(computedTotal)}</p>
                {savings > 0 && (
                  <p className="text-xs text-green-600">You saved ₹{Math.round(savings)}</p>
                )}
              </div>
              <Button
                className="h-12 rounded-xl bg-orange-500 px-8 font-medium text-white hover:bg-orange-600"
          ...
          <BillBreakdownSheet open={billOpen} onOpenChange={setBillOpen} values={billValues} />
        </div>
      </div>
    </CustomerLayout>
  );
