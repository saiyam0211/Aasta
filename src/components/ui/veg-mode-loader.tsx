...
interface VegModeLoaderProps {
  isVisible: boolean;
  isEntering: boolean; // true for "Entering Veg Mode", false for "Exiting Veg Mode"
}

const VegModeLoader: React.FC<VegModeLoaderProps> = ({ isVisible, isEntering }) => {
  const text = isEntering ? 'Entering Veg Mode' : 'Exiting Veg Mode';
  const bgColor = isEntering ? '#002a01' : '#e8e8e8'; // Green for entering, black for exiting
  const textColor = isEntering ? '#ffffff' : '#000000';
  const shadowColor = isEntering ? '#aaaaaa' : '#000000';
        ...
        >
          ...
          <style jsx>{`
            ...
            .loader .line::after {
              ...
              background-color: var(--text-color);
              border-radius: 0.05em;
              transform: ${isEntering ? 'translateX(-90%)' : 'translateX(90%)'};
            ...
            @keyframes scrolling {
              0% {
                transform: ${isEntering ? 'translateX(-100%)' : 'translateX(100%)'};
              }
              100% {
                transform: ${isEntering ? 'translateX(100%)' : 'translateX(-100%)'};
              }
            ...
            @keyframes shadow {
              0% {
                background-position: ${isEntering ? '-98% 0' : '102% 0'};
              }
              100% {
                background-position: ${isEntering ? '102% 0' : '-98% 0'};
              }
            }
          `}</style>
        </motion.div>
