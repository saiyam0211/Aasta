...
export function BillBreakdownSheet({ open, onOpenChange, values }: BillBreakdownSheetProps) {
	const {
		originalSubtotal,
		aastaSubtotal,
		platformFee,
				...
				<Dialog.Content className="sheet-content data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:slide-in-from-bottom data-[state=closed]:slide-out-to-bottom fixed bottom-0 left-1/2 z-[101] w-full max-w-md -translate-x-1/2 rounded-t-3xl bg-white shadow-2xl">
					...
					<div className="max-h-[80vh] overflow-auto p-5 pt-6">
						<div className="mb-4 flex items-center justify-between">
							<Dialog.Title className="text-lg font-semibold">Bill Summary</Dialog.Title>
						...
						<div className="space-y-3">
							...
							<div className="flex items-center justify-between">
								...
								<span className="text-gray-400 line-through">₹{originalSubtotal.toFixed(2)}</span>
							</div>
							<div className="flex items-center justify-between">
								<span className="font-medium">Aasta price</span>
								<span className="font-semibold">₹{aastaSubtotal.toFixed(2)}</span>
							</div>
							...
							<div className="mt-2 border-t pt-2">
								...
								<div className="flex items-center justify-between">
									...
									<span>
										<span className="mr-2 text-gray-400 line-through">₹{packagingFeeOriginal.toFixed(2)}</span>
										<span className="font-medium">₹{packagingFeeDisplay.toFixed(2)}</span>
									</span>
								</div>
								<div className="flex items-center justify-between">
						...
						<div className="mt-3 border-t pt-3">
							...
							{typeof savings === 'number' && savings > 0 && (
								<div className="mt-2 rounded-xl bg-blue-50 p-2 text-sm text-blue-700">You saved ₹{savings.toFixed(2)} on this order</div>
							)}
						</div>
						</div>
					</div>
